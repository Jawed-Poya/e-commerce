...
class ApiClient {
    ...
    private async getBlob(url: string, params?: object) {
        // Some IIS/reverse-proxy paths can occasionally complete a successful document
        // request before the generated body reaches the browser. Read raw bytes and retry
        // only an otherwise-successful empty response once. The retry is serial, so a
        // normal successful download still performs exactly one request.
        ...
        throw new Error("The document download completed without file content. Please try again.");
    }
...
function createDownloadUrl(path: string, params?: object, attempt = 0) {

...
const navigation: ProtectedGroup[] = [
    { label: "Overview", items: [
        ...
        { title: "Financial reports", url: "/reports", icon: <BarChart3 />, permission: Permissions.FinancialReportsView },
    ]},
    ...
    { label: "Administration", items: [
        { title: "Company settings", url: "/company", icon: <Building2 />, permission: Permissions.CompanyProfileManage },
        ...
        { title: "Trash", url: "/trash", icon: <Trash2 />, permission: Permissions.CompanyTrashManage },
    ]},
];
...
export function AppSidebar(props: React.ComponentProps<typeof Sidebar>) {
    ...
    const groups: NavigationGroup[] = navigation.map((group) => ({
        label: group.label,
        items: group.items
            .map((item) => ({ ...item, items: item.items?.filter((child) => !child.permission || hasPermission(user, child.permission)) }))

export type CompanyLanguage = "en" | "dr" | "ps";

export interface CompanyFontOption {
    value: string;
    label: string;
    bundled: boolean;
}

export const companyFontOptions: Record<CompanyLanguage, CompanyFontOption[]> = {
    en: [
...
export function resolveCompanyFontStack(language: CompanyLanguage, selected: string) {
...
export function resolveCompanyHeadingStack(language: CompanyLanguage, selected: string) {
    if (language === "en")
        return `"Manrope Variable", ${resolveCompanyFontStack(language, selected)}`;
    if (language === "dr")
        return `"Noto Naskh Arabic Variable", ${resolveCompanyFontStack(language, selected)}`;
    return `"Noto Naskh Arabic Variable", ${resolveCompanyFontStack(language, selected)}`;
}

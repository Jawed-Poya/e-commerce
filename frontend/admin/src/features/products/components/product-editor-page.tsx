    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,.65fr)]">
      <div className="space-y-6">
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><PackagePlus className="size-5" />Product information</CardTitle></CardHeader><CardContent className="grid min-w-0 gap-4 lg:grid-cols-2">
          ...
          <label className="flex items-center gap-3 rounded-lg border p-3"><Checkbox checked={form.isFeatured} onCheckedChange={v => setForm(x => ({ ...x, isFeatured: v === true }))} /><span><strong className="block text-sm">{t("products.pinToggleTitle")}</strong><span className="text-xs text-muted-foreground">{t("products.pinToggleHelp")}</span></span></label>
        </CardContent></Card>

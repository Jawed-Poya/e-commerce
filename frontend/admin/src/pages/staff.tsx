...
export default function StaffPage() {
    ...
    const { data: staff } = useOperationQuery(operationKeys.staff, operationsService.staff, canViewPayroll && salaryOpen);
    const [staffPage, setStaffPage] = useState(1);
    const [staffPageSize, setStaffPageSize] = useState(20);
    const [salaryPage, setSalaryPage] = useState(1);
    const [salaryPageSize, setSalaryPageSize] = useState(20);
    const { data: staffPageData, isLoading } = useOperationQuery(
        operationKeys.staffPage("", staffPage, staffPageSize),
        () => operationsService.staffPage("", staffPage, staffPageSize),
    );
    const { data: salaryPageData, isLoading: salariesLoading } = useOperationQuery(
        operationKeys.salaryPage(salaryPage, salaryPageSize),
        () => operationsService.salaries(salaryPage, salaryPageSize),
        canViewPayroll,
    );
    const staffList = staffPageData?.items;
    const salaries = salaryPageData?.items;
    ...
    return <div className="space-y-6">
        ...
        <Dialog open={salaryOpen} onOpenChange={setSalaryOpen}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>Create salary obligation</DialogTitle><DialogDescription>Create one monthly salary record, optionally pay part now, and settle the remainder later.</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><Field label="Staff member"><SimpleCombobox<number> value={salary.staffId || null} onValueChange={(value) => setSalary((x) => ({ ...x, staffId: value ?? 0, paidAmount: 0 }))} options={(staff ?? []).filter((item) => item.isActive).map((item) => ({ value: item.id, label: item.fullName, description: `${item.position || tr("Staff")} · ${money(item.baseSalary)}` }))} placeholder="Select staff" emptyText="No active staff found." /></Field><Field label="Period"><div className="grid grid-cols-2 gap-2"><SimpleCombobox<number> value={salary.periodMonth} onValueChange={(value) => setSalary((x) => ({ ...x, periodMonth: value ?? 1 }))} options={Array.from({ length: 12 }, (_, index) => ({ value: index + 1, label: monthName(index + 1, locale) }))} placeholder="Select month" /><Input type="number" min={2000} max={2100} value={salary.periodYear} onChange={(event) => setSalary((x) => ({ ...x, periodYear: Number(event.target.value) }))} /></div></Field><Field label="Bonus"><Input type="number" min={0} step="0.01" value={salary.bonus} onChange={(event) => setSalary((x) => ({ ...x, bonus: Number(event.target.value), paidAmount: 0 }))} /></Field><Field label="Deduction"><Input type="number" min={0} step="0.01" value={salary.deduction} onChange={(event) => setSalary((x) => ({ ...x, deduction: Number(event.target.value), paidAmount: 0 }))} /></Field><Field label="Opening paid amount"><Input type="number" min={0} max={netSalary} step="0.01" value={salary.paidAmount} onChange={(event) => setSalary((x) => ({ ...x, paidAmount: Number(event.target.value) }))} /></Field><Field label="Payment date"><Input type="date" value={salary.paidDate} onChange={(event) => setSalary((x) => ({ ...x, paidDate: event.target.value }))} /></Field><Field label="Payment method"><SimpleCombobox value={salary.paymentMethod} onValueChange={(value) => setSalary((x) => ({ ...x, paymentMethod: value ?? "Cash" }))} options={["Cash", "Bank transfer", "Cheque", "Other"].map((value) => ({ value, label: value }))} placeholder="Select payment method" /></Field><Field label="Reference"><Input value={salary.referenceNumber} onChange={(event) => setSalary((x) => ({ ...x, referenceNumber: event.target.value }))} /></Field><div className="grid gap-3 rounded-lg border bg-muted/30 p-4 sm:col-span-2 sm:grid-cols-3"><Summary label="Net salary" value={netSalary} /><Summary label="Pay now" value={salary.paidAmount} /><Summary label="Remaining" value={Math.max(0, netSalary - salary.paidAmount)} /></div><div className="space-y-2 sm:col-span-2"><Label>Notes</Label><Textarea rows={3} value={salary.notes} onChange={(event) => setSalary((x) => ({ ...x, notes: event.target.value }))} /></div></div><DialogFooter><Button variant="outline" onClick={() => setSalaryOpen(false)}>Cancel</Button><Button onClick={() => void saveSalary()} disabled={saving || !salary.staffId}><Save className="me-2 size-4" />Create salary</Button></DialogFooter></DialogContent></Dialog>
        {selectedSalary ? <PaymentLedgerDialog<SalaryPayment> open={Boolean(selectedSalary)} onOpenChange={(next) => { if (!next) setSelectedSalary(null); }} title="Salary payments" description={`${selectedSalary.staffName} · ${monthName(selectedSalary.periodMonth, locale)} ${selectedSalary.periodYear}`} documentNumber={`PAY-${selectedSalary.id}`} total={selectedSalary.netAmount} paidAmount={selectedSalary.paidAmount} remainingAmount={selectedSalary.remainingAmount} paymentStatus={selectedSalary.paymentStatus} queryKey={operationKeys.salaryPayments(selectedSalary.id)} loadPayments={() => operationsService.salaryPayments(selectedSalary.id)} addPayment={(body) => operationsService.addSalaryPayment(selectedSalary.id, body)} onDocumentUpdated={setSelectedSalary} invalidate={[operationKeys.salaries, operationKeys.summary]} canManage={canManagePayroll} /> : null}
    </div>;
}
...
function monthName(month: number, locale?: string) { return new Intl.DateTimeFormat(locale, { month: "long", timeZone: "UTC" }).format(new Date(Date.UTC(2024, month - 1, 1))); }
function detectedRole(position: string | null) { const value = position?.toLowerCase() ?? ""; if (/sales|seller|salesman/.test(value)) return "Salesperson"; if (/book|account|finance|cashier/.test(value)) return "Bookkeeper"; return null; }

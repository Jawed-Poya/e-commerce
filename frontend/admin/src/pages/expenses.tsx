...
function ExpenseSheetRow({ row, index, categories, onChange, onRemove }: { row: ExpenseDraft; index: number; categories: ExpenseCategory[]; onChange: (row: ExpenseDraft) => void; onRemove: () => void }) {
    const field = <K extends keyof ExpenseDraft>(key: K, value: ExpenseDraft[K]) => onChange({ ...row, [key]: value });
    return <div className="grid grid-cols-[45px_140px_190px_130px_180px_160px_160px_1fr_45px] gap-2 border-b p-2 last:border-b-0"><span className="self-center text-center font-semibold text-muted-foreground">{index + 1}</span><Input type="date" value={row.expenseDate} onChange={(event) => field("expenseDate", event.target.value)} /><SimpleCombobox<string> value={row.categoryId || null} onValueChange={(value) => field("categoryId", value ?? "")} options={categories.map((category) => ({ value: String(category.id), label: category.name }))} placeholder="Select category" emptyText="No expense categories found." /><Input type="number" min={0.01} step="0.01" value={row.amount} onChange={(event) => field("amount", Number(event.target.value))} /><Input value={row.vendor} onChange={(event) => field("vendor", event.target.value)} /><SimpleCombobox<string> value={row.paymentMethod} onValueChange={(value) => field("paymentMethod", value ?? "Cash")} options={["Cash", "Card", "Bank transfer", "Mobile money", "Other"].map((value) => ({ value, label: value }))} placeholder="Select payment method" /><Input value={row.referenceNumber} onChange={(event) => field("referenceNumber", event.target.value)} /><Input value={row.description} onChange={(event) => field("description", event.target.value)} /><Button size="icon" variant="ghost" onClick={onRemove}><Trash2 className="size-4 text-destructive" /></Button></div>;
}

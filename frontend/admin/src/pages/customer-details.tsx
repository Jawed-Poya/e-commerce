...
import { Activity, ArrowLeft, Eye, Globe2, Mail, MapPin, Phone, Radio, RefreshCw, Save, Search, UserRound } from "lucide-react";
...
import { useCustomerPresenceStream, type CustomerRealtimeStatus } from "@/features/customers/customer-presence-stream";
...
export default function CustomerDetailsPage() {
    ...
    const validCustomerId = Number.isFinite(id) && id > 0;
    const realtimeStatus = useCustomerPresenceStream(validCustomerId ? id : undefined);
    const query = useQuery({ queryKey: customerQueryKeys.details(id), queryFn: () => customerService.getCustomer(id), enabled: validCustomerId });
    const engagement = useQuery({
        queryKey: customerQueryKeys.engagement(id),
    ...
    return <div className="space-y-5"><PageHeader title={name} description={`Customer since ${new Date(customer.createdAt).toLocaleDateString()}`} actions={<Link to="/customers" className="inline-flex h-8 items-center gap-1 border px-2.5 text-xs hover:bg-muted"><ArrowLeft className="size-4" />Back</Link>} />
        ...
        <CustomerActivityCard engagement={engagement.data} loading={engagement.isLoading} realtimeStatus={realtimeStatus} refreshing={engagement.isFetching} onRefresh={() => void engagement.refetch()} />
...
function CustomerActivityCard({
    ...
    loading: boolean;
    realtimeStatus: CustomerRealtimeStatus;
    refreshing: boolean;
    onRefresh: () => void;
}) {
    const { t } = useI18n();
        ...
        <Card className="overflow-hidden">
            <CardHeader className="border-b">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <CardTitle>{t("customers.liveActivity")}</CardTitle>
                        <p className="mt-1 text-sm text-muted-foreground">
                            {t("customers.liveActivityDescription")}
                        </p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        >
                            <Radio className={isLive ? "size-3.5 animate-pulse" : "size-3.5"} />
                            {isLive ? t("customers.liveCrm") : t("customers.pollingFallback")}
                        </Badge>
                        <Badge variant="outline" className={engagement?.isOnline ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : "text-muted-foreground"}>
                            <span className={`me-2 size-2 rounded-full ${engagement?.isOnline ? "animate-pulse bg-emerald-500" : "bg-muted-foreground/50"}`} />
                            {engagement?.isOnline ? `Online · ${Math.max(1, toFiniteNumber(engagement.activeSessions))} session${toFiniteNumber(engagement.activeSessions) === 1 ? "" : "s"}` : "Offline"}
                        </Badge>
                        <Button
                            type="button"
                            variant="outline"
            ...
            <CardContent className="space-y-4 pt-5">
                {loading ? (
                    <p className="text-sm text-muted-foreground">Loading customer activity…</p>
                ) : (
                    <>
                        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                            {metrics.map(({ icon: Icon, label, value }) => (
                                <div key={label} className="border bg-muted/20 p-3">
                                    ...
                                    <p className="mt-2 text-xl font-bold tabular-nums">{value.toLocaleString()}</p>
                                </div>
                            ))}
                        </div>
                        <div className="grid gap-3 lg:grid-cols-3">
                            <ActivityDetail label={engagement?.isOnline ? "Currently viewing" : "Last visited page"} value={engagement?.pageTitle || friendlyPath(engagement?.currentPath)} help={engagement?.currentPath ?? "No storefront activity recorded"} />
                            <ActivityDetail label="Last seen" value={formatSeen(engagement?.lastSeenAt)} help={engagement?.isOnline ? "Presence refreshes every few seconds" : "The customer is not currently active"} />
                            <ActivityDetail label="Latest search" value={engagement?.lastSearchTerm || "—"} help="Useful for follow-up and stock decisions" />
                        </div>
                    </>
                )}
            </CardContent>

...
export default function JournalVouchersPage() {
    ...
    return <div className="space-y-6">
        ...
        <Card className="shadow-none">
            ...
            <CardContent className="space-y-4 p-4 sm:p-6"><div className="grid gap-3 lg:grid-cols-[minmax(240px,1fr)_220px_180px_180px]"><div className="relative"><Search className="absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="ps-9" value={search} onChange={(event) => { setSearch(event.target.value); resetPage(); }} placeholder="Voucher, source, party, reference…" /></div><FilterSelect value={type} onChange={(value) => { setType(value as JournalVoucherType | "all"); resetPage(); }}><option value="all">All voucher types</option>{voucherTypes.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</FilterSelect><FilterSelect value={origin} onChange={(value) => { setOrigin(value as typeof origin); resetPage(); }}><option value="all">All origins</option><option value="system">Workflow generated</option><option value="manual">Manual adjustment</option></FilterSelect><FilterSelect value={status} onChange={(value) => { setStatus(value as JournalVoucherStatus | "all"); resetPage(); }}><option value="all">All statuses</option><option value="Posted">Posted</option><option value="Reversed">Reversed</option></FilterSelect></div></CardContent>

        ...
        <div className="space-y-6">
            ...
            <section className="space-y-4 border p-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <ToggleGroup variant="outline" value={[group]} onValueChange={value => { if (value[0]) { setGroup(value[0]); setPage(1); } }} className="flex flex-wrap justify-start">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="relative min-w-0 flex-1 sm:w-72"><Search className="absolute start-3 top-2.5 size-4 text-muted-foreground" /><Input className="ps-9" value={search} onChange={event => { setSearch(event.target.value); setPage(1); }} placeholder={t("types.search")} /></div>
                ...
                <GeneralTypesTable rows={pagedRows} allRows={allRows} isLoading={isLoading} isError={isError} onRetry={() => refetch()} />
                <ListPagination page={page} pageSize={pageSize} totalCount={rows.length} disabled={isLoading} onPageChange={setPage} onPageSizeChange={(size) => { setPageSize(size); setPage(1); }} />
            </section>

...
export default function CustomersPage() {
    ...
    return <div className="space-y-5">
        ...
        <Dialog open={open} onOpenChange={setOpen}><DialogContent className="sm:max-w-lg"><form onSubmit={submit}><DialogHeader><DialogTitle>Add customer</DialogTitle><DialogDescription>Create a customer without placing an order.</DialogDescription></DialogHeader><div className="my-5 grid gap-4 sm:grid-cols-2"><Field label="First name" value={form.firstName} onChange={value => setForm(x => ({ ...x, firstName: value }))} required /><Field label="Last name" value={form.lastName} onChange={value => setForm(x => ({ ...x, lastName: value }))} /><Field label="Phone" value={form.phone} onChange={value => setForm(x => ({ ...x, phone: value }))} required /><Field label="Email" value={form.email} onChange={value => setForm(x => ({ ...x, email: value }))} /><div className="space-y-2"><Label>Customer type</Label><select className="flex h-9 w-full border border-input bg-transparent px-3 py-1 text-sm shadow-xs outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50" value={form.customerTypeId || String(lookups?.defaultCustomerTypeId ?? "")} onChange={e => setForm(x => ({ ...x, customerTypeId: e.target.value }))}>{lookups?.customerTypes.map(type => <option key={type.id} value={type.id}>{type.name}{type.id === lookups.defaultCustomerTypeId ? " (Default / guests)" : ""}</option>)}</select></div><div className="sm:col-span-2 space-y-2"><Label>Address</Label><Textarea value={form.address} onChange={e => setForm(x => ({ ...x, address: e.target.value }))} /></div></div><DialogFooter><Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button type="submit" disabled={create.isPending || !form.firstName.trim() || !form.phone.trim()}>{create.isPending ? "Saving..." : "Create customer"}</Button></DialogFooter></form></DialogContent></Dialog>
    </div>;
}

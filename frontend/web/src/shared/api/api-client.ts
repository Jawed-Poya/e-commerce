const API_URL = import.meta.env.VITE_API_BASE_URL ?? 'http://localhost:5188/api'
...
export async function apiGet<T>(path: string, params?: Record<string, string | number | boolean | undefined>) {
  ...
  if (!response.ok) throw new Error(response.status === 404 ? 'Not found' : 'The store could not load this information.')
  const payload = await response.json() as ApiEnvelope<T> | T
  return (typeof payload === 'object' && payload !== null && 'data' in payload ? payload.data : payload) as T
}

export function imageUrl(path?: string | null) { if (!path) return null; return /^https?:/.test(path) ? path : `${apiOrigin}${path.startsWith('/') ? path : `/${path}`}` }

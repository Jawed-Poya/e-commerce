import type { Messages } from "../types";
import { en } from "./en";

export const dr: Messages = {
    ...en,
    "language.en": "انگلیسی",
    "language.dr": "دری",
    "language.ps": "پشتو",
    "common.loading": "در حال بارگذاری...",
    "common.retry": "دوباره تلاش کنید",
    "common.open": "باز کردن",
    "notifications.markRead": "علامت‌گذاری به‌عنوان خوانده‌شده",
    "notifications.clear": "پاک‌کردن",
    "notifications.clearConfirm": "همه پاک شود؟",
    "pwa.installed": "برنامه نصب شد",
    "nav.home": "خانه",
    "nav.shop": "فروشگاه",
    "nav.categories": "دسته‌بندی‌ها",
    "nav.deals": "تخفیف‌ها",
    "nav.trackOrder": "پیگیری سفارش",
    "nav.featured": "محصولات ویژه",
    "header.welcome": "به بازار {company} خوش آمدید",
    "header.help": "مرکز کمک",
    "header.secure": "خرید امن و قابل اعتماد",
    "header.pharmacyMarketplace": "بازار آنلاین داروخانه",
    "header.searchPlaceholder": "جستجوی محصولات، برندها و دسته‌بندی‌ها...",
    "common.search": "جستجو",
    "common.shareStore": "اشتراک این فروشگاه",
    "common.shareStoreText": "فروشگاه آنلاین ما را با این پیوند رسمی باز کنید.",
    "common.linkCopied": "پیوند فروشگاه کپی شد",
    "common.copyStoreLink": "این پیوند فروشگاه را کپی کنید",
    "common.viewAll": "مشاهده همه",
    "common.home": "خانه",
    "common.products": "محصولات",
    "common.cart": "سبد خرید",
    "common.wishlist": "علاقه‌مندی‌ها",
    "common.account": "حساب",
    "common.login": "ورود",
    "common.logout": "خروج",
    "common.save": "ذخیره",
    "common.cancel": "لغو",
    "common.remove": "حذف",
    "common.submit": "ارسال",
    "common.backToShop": "بازگشت به فروشگاه",
    "common.scrollToTop": "رفتن به بالای صفحه",
    "home.fastDelivery": "تحویل سریع",
    "home.safeReliable": "امن و قابل اعتماد",
    "home.pharmacyEyebrow": "خدمات داروخانه، ساده‌تر",
    "home.pharmacyHeroTitle": "محصولات صحی قابل اعتماد، با نظم روشن.",
    "home.pharmacyHeroDescription": "داروها، محصولات سلامتی و مراقبت شخصی را با قیمت‌گذاری شفاف واحدها، موجودی زنده و سفارش امن مرور کنید.",
    "home.shopPharmacy": "خرید از داروخانه",
    "home.clearUnits": "واحدهای فروش روشن",
    "home.flexibleUnits": "واحدهای انعطاف‌پذیر داروخانه",
    "home.unitShoppingTitle": "واحد مناسب سفارش خود را انتخاب کنید.",
    "home.unitShoppingDescription": "موجودی در یک واحد پایه نگهداری می‌شود، اما مشتری می‌تواند بر اساس دانه، ورق، جعبه، بوتل یا هر واحد تنظیم‌شده خرید کند.",
    "home.easyReturns": "بازگشت آسان",
    "home.simplePolicy": "قوانین ساده",
    "home.secureShopping": "خرید امن",
    "home.protectedCheckout": "پرداخت محافظت‌شده",
    "home.trustedCatalog": "کاتالوگ قابل اعتماد",
    "home.updatedInfo": "اطلاعات به‌روز محصولات",
    "home.liveAvailability": "موجودی زنده",
    "home.inStockNow": "محصولات موجود را خریداری کنید",
    "home.limitedOffers": "پیشنهادهای محدود",
    "home.flashDeals": "تخفیف‌های ویژه",
    "home.flashDealsDescription": "محصولات محبوب با قیمت کاهش‌یافته.",
    "home.browseCollection": "مجموعه ما را ببینید",
    "home.shopByCategory": "خرید بر اساس دسته‌بندی",
    "home.categoryDescription": "با مرور دسته‌بندی‌های اصلی سریع‌تر محصول مورد نیاز خود را پیدا کنید.",
    "home.exploreCategory": "مشاهده دسته‌بندی",
    "home.featuredCatalog": "ویژه از کاتالوگ",
    "home.selectedForYou": "محصولاتی که ویژه برای شما انتخاب شده‌اند.",
    "home.selectedDescription": "تصاویر، قیمت فعلی، موجودی و جزئیات مهم محصول را در یک جا ببینید.",
    "home.viewProduct": "مشاهده محصول",
    "home.newArrivals": "محصولات تازه",
    "home.freshProducts": "محصولات تازه به‌طور منظم اضافه می‌شوند",
    "home.freshDescription": "جدیدترین محصولات افزوده‌شده به کاتالوگ را ببینید.",
    "home.freeDelivery": "تحویل رایگان",
    "home.qualifyingOrders": "برای سفارش‌های واجد شرایط بالاتر از {amount}",
    "home.everyOrderFree": "تحویل رایگان برای همه سفارش‌ها",
    "home.flatDelivery": "هزینه ثابت تحویل {amount}",
    "home.deliveryDescription": "از تحویل آسان برای خریدهای واجد شرایط استفاده کنید.",
    "home.featuredProducts": "محصولات ویژه",
    "home.justForYou": "فقط برای شما",
    "home.justForYouDescription": "محصولات منتخب از تازه‌ترین موارد کاتالوگ را کشف کنید.",
    "home.productsUnavailable": "محصولات فعلاً در دسترس نیستند",
    "home.productsUnavailableDescription": "کاتالوگ محصولات موقتاً در دسترس نیست. بعداً دوباره تلاش کنید.",
    "footer.title": "با {company} هوشمندانه‌تر خرید کنید",
    "footer.description": "محصولات قابل اعتماد، خرید امن و تجربه‌ای ساده برای هر سفارش.",
    "footer.secureCheckout": "پرداخت امن",
    "footer.fastDelivery": "تحویل سریع",
    "footer.easyReturns": "بازگشت آسان",
    "footer.about": "محصولات قابل اعتماد، موجودی قابل مشاهده و تجربه خرید آسان.",
    "footer.shop": "فروشگاه",
    "footer.customerCare": "خدمات مشتری",
    "footer.allProducts": "همه محصولات",
    "footer.newArrivals": "محصولات تازه",
    "footer.categories": "دسته‌بندی‌ها",
    "footer.contactSupport": "تماس با پشتیبانی",
    "footer.deliveryReturns": "تحویل و بازگشت",
    "footer.privacy": "حریم خصوصی",
    "footer.rights": "تمام حقوق محفوظ است.",
    "footer.betterExperience": "ساخته‌شده برای تجربه بهتر خرید آنلاین.",
    "notifications.title": "اعلان‌ها",
    "notifications.description": "تغییر قیمت، موجودی و فعالیت سبد خرید.",
    "notifications.live": "زنده",
    "notifications.fallback": "پشتیبان",
    "notifications.enableDescription": "اعلان مرورگر را برای تغییرات مهم قیمت، موجودی و سبد خرید {company} فعال کنید.",
    "notifications.enable": "فعال‌سازی اعلان",
    "notifications.blocked": "اعلان‌های مرورگر مسدود است. برای دریافت به‌روزرسانی‌ها هنگام بسته بودن صفحه، اعلان‌ها را در تنظیمات مرورگر اجازه دهید.",
    "notifications.pushConnecting": "در حال اتصال اعلان‌های پس‌زمینه…",
    "notifications.pushError": "اجازهٔ اعلان داده شده، اما تحویل پس‌زمینه وصل نشد.",
    "notifications.retry": "تلاش دوباره",
    "notifications.emptyTitle": "هنوز اعلانی نیست",
    "notifications.emptyDescription": "تغییرات قیمت به‌صورت خودکار اطلاع داده می‌شوند. محصولات دنبال‌شده شما برای تغییر موجودی نیز بررسی می‌شوند.",
    "notifications.cartAddedTitle": "به سبد خرید افزوده شد",
    "notifications.cartAddedMessage": "{product} به سبد خرید شما افزوده شد.",
    "product.notFound": "محصول یافت نشد",
    "product.notFoundDescription": "این محصول شاید حذف شده یا دیگر در دسترس نباشد.",
    "product.featured": "ویژه",
    "product.inStock": "موجود",
    "product.unavailable": "ناموجود",
    "product.addToCart": "افزودن به سبد",
    "product.inCart": "در سبد",
    "product.notifyMe": "به من اطلاع بده",
    "product.enableAlerts": "فعال‌سازی اعلان مرورگر",
    "product.alertsEnabled": "اعلان‌ها فعال است",
    "product.alertsBlocked": "اعلان‌ها مسدود است",
    "product.alertsUnavailable": "اعلان در دسترس نیست",
    "product.alertTrackingDescription": "تغییر قیمت و موجودی در اعلان‌های {company} شما دنبال می‌شود.",
    "product.minimumQuantity": "حداقل {count}",
    "product.maximumQuantity": "حداکثر {count}",
    "product.saved": "ذخیره شد",
    "product.details": "جزئیات",
    "product.pin": "سنجاق در بالای فهرست",
    "product.unpin": "برداشتن از بالای فهرست",
    "product.information": "اطلاعات محصول",
    "product.category": "دسته‌بندی",
    "product.strength": "قدرت / دوز",
    "product.brand": "برند",
    "product.unit": "واحد",
    "product.chooseSellingUnit": "واحد فروش را انتخاب کنید",
    "product.unitConversionHelp": "موجودی همیشه با واحد اساسی نگهداری و به‌صورت خودکار تبدیل می‌شود.",
    "product.baseUnit": "واحد اساسی: {unit}",
    "product.baseInventoryUnit": "واحد اساسی موجودی",
    "product.unitEquation": "۱ {unit} = {factor} {baseUnit}",
    "product.perUnit": "برای هر {unit}",
    "product.barcode": "بارکد",
    "product.totalPrice": "قیمت مجموعی",
    "product.noPrice": "بدون قیمت",
    "reviews.title": "دیدگاه مشتریان",
    "reviews.summary": "بر اساس {count} دیدگاه",
    "reviews.write": "ثبت دیدگاه",
    "reviews.edit": "ویرایش دیدگاه شما",
    "reviews.rating": "امتیاز",
    "reviews.comment": "نظر",
    "reviews.commentPlaceholder": "تجربه خود را از این محصول بنویسید...",
    "reviews.submit": "ارسال دیدگاه",
    "reviews.update": "به‌روزرسانی دیدگاه",
    "reviews.login": "برای ثبت دیدگاه وارد شوید.",
    "reviews.pending": "دیدگاه شما در انتظار تأیید است.",
    "reviews.verified": "خرید تأییدشده",
    "reviews.empty": "هنوز دیدگاه تأییدشده‌ای وجود ندارد.",
    "reviews.delete": "حذف دیدگاه",
    "pwa.install": "نصب برنامه",
    "pwa.installTitle": "نصب برنامه فروشگاه",
    "pwa.installDescription": "از صفحه اصلی دستگاه سریع‌تر خرید کنید.",
    "pwa.browserInstructions": "منوی مرورگر را باز کنید و گزینه نصب برنامه یا افزودن به صفحه اصلی را انتخاب کنید. نصب در مرورگر پشتیبانی‌شده و اتصال امن فعال می‌شود.",
    "pwa.iosInstructions": "در Safari روی اشتراک‌گذاری بزنید، سپس افزودن به صفحه اصلی را انتخاب کنید.",
    "pwa.iosShareHint": "اشتراک‌گذاری ← افزودن به صفحه اصلی",
    "mobileApp.download": "دانلود برنامه موبایل",
    "mobileApp.downloadTitle": "برنامه موبایل EasyCart را بگیرید",
    "mobileApp.downloadDescription": "برای خرید سریع‌تر و اعلان‌های موبایل، برنامه اصلی را نصب کنید.",
    "google.loading": "در حال بارگذاری ورود با گوگل",
    "google.notConfigured": "ورود با گوگل در این سرور تنظیم نشده است.",
    "google.configurationUnavailable": "تنظیمات ورود با گوگل از API بارگذاری نشد.",
    "google.scriptUnavailable": "ورود با گوگل بارگذاری نشد. شبکه یا تنظیمات حریم خصوصی مرورگر را بررسی کنید.",
    "google.originUnavailable": "آدرس این وب‌سایت در مشتری OAuth گوگل مجاز نشده است.",
    "google.insecureContext": "هنگام باز کردن فروشگاه از کمپیوتر دیگر، ورود با گوگل به HTTPS نیاز دارد.",
    "carousel.previous": "دسته‌بندی‌های قبلی",
    "carousel.next": "دسته‌بندی‌های بعدی",
    "notFound.title": "صفحه یافت نشد",
    "notFound.description": "صفحه مورد نظر منتقل شده یا دیگر وجود ندارد.",

    "common.free": "رایگان",
    "common.close": "بستن",
    "common.clearAll": "پاک کردن همه",
    "common.continueShopping": "ادامه خرید",
    "common.notSet": "تنظیم نشده",
    "common.general": "عمومی",
    "common.phone": "تلفن",
    "common.email": "ایمیل",
    "common.password": "رمز عبور",
    "common.firstName": "نام",
    "common.lastName": "تخلص",
    "common.order": "سفارش",
    "common.payment": "پرداخت",
    "common.delivery": "تحویل",
    "common.subtotal": "جمع فرعی",
    "common.total": "مجموع",
    "common.status": "وضعیت",
    "mobile.mainMenu": "منوی اصلی",
    "mobile.shopByCategory": "خرید بر اساس دسته‌بندی",
    "mobile.needHelp": "به کمک در خرید نیاز دارید؟",
    "mobile.helpDescription": "برای راهنمایی محصول و سفارش با پشتیبانی تماس بگیرید.",
    "product.defaultDescription": "محصولی با انتخاب دقیق، قیمت شفاف و موجودی زنده.",
    "product.currentPrice": "قیمت فعلی",
    "product.availability": "موجودی",
    "product.availableCount": "{count} موجود",
    "product.soldOut": "تمام شده",
    "product.readyToOrder": "موجود و آماده سفارش",
    "product.stockDescription": "فعلاً {count} عدد برای خرید موجود است.",
    "product.unavailableDescription": "این محصول فعلاً قابل سفارش نیست. بعداً دوباره بررسی کنید.",
    "product.views": "{count} بازدید",
    "product.defaultTierPrice": "قیمت پیش‌فرض / {type}",
    "product.tierPrice": "قیمت {type}",
    "product.general": "عمومی",
    "product.customer": "مشتری",
    "wishlist.add": "افزودن به علاقه‌مندی‌ها",
    "wishlist.remove": "حذف از علاقه‌مندی‌ها",
    "wishlist.savedProducts": "محصولات ذخیره‌شده",
    "wishlist.emptyTitle": "فهرست علاقه‌مندی‌های شما خالی است.",
    "wishlist.emptyDescription": "با دکمه قلب هر محصول را برای بعد ذخیره کنید.",
    "wishlist.explore": "مشاهده محصولات",
    "wishlist.savedForLater": "ذخیره‌شده برای بعد",
    "wishlist.loading": "در حال بارگذاری محصولات ذخیره‌شده...",
    "wishlist.loadError": "محصولات ذخیره‌شده بارگذاری نشد. دوباره تلاش کنید.",
    "catalog.shop": "فروشگاه",
    "catalog.curated": "بازار منتخب",
    "catalog.heroTitle": "محصول مناسب خود را پیدا کنید.",
    "catalog.heroDescription": "کاتالوگ کامل را با قیمت روشن، موجودی زنده، جزئیات مفید و فیلترهای انعطاف‌پذیر مرور کنید.",
    "catalog.availableNow": "فعلاً موجود",
    "catalog.productsInCatalog": "محصول در کاتالوگ",
    "catalog.any": "همه",
    "catalog.sort": "مرتب‌سازی",
    "catalog.clearFilters": "پاک کردن فیلترها",
    "catalog.searchLabel": "جستجو: {value}",
    "catalog.searchPlaceholder": "جستجو با نام محصول یا بارکد...",
    "catalog.filters": "فیلترها",
    "catalog.category": "دسته‌بندی",
    "catalog.brand": "برند",
    "catalog.unit": "واحد",
    "catalog.priceRange": "محدوده قیمت",
    "catalog.availability": "موجودی",
    "catalog.collection": "مجموعه",
    "catalog.allCategories": "همه دسته‌بندی‌ها",
    "catalog.allBrands": "همه برندها",
    "catalog.allUnits": "همه واحدها",
    "catalog.allProducts": "همه محصولات",
    "catalog.inStock": "موجود",
    "catalog.outOfStock": "ناموجود",
    "catalog.featured": "ویژه",
    "catalog.minimum": "حداقل",
    "catalog.maximum": "حداکثر",
    "catalog.unavailableTitle": "کاتالوگ در دسترس نیست",
    "catalog.noMatches": "محصولی مطابق جستجو یافت نشد",
    "catalog.noMatchesDescription": "جستجو را تغییر دهید یا یکی از فیلترها را حذف کنید.",
    "cart.title": "سبد خرید",
    "cart.emptyTitle": "سبد خرید شما خالی است",
    "cart.emptyDescription": "محصولات افزوده‌شده در اینجا نمایش داده می‌شوند.",
    "cart.syncAccountRequired": "برای نگهداری این سبد در وب و موبایل وارد شوید",
    "cart.syncing": "سبد در حال همگام‌سازی است…",
    "cart.synced": "در وب و موبایل همگام شد",
    "cart.syncOffline": "همگام‌سازی متوقف است — تلاش دوباره خودکار انجام می‌شود",
    "cart.delivery": "تحویل",
    "cart.freeDelivery": "تحویل رایگان",
    "cart.closeToFreeDelivery": "به تحویل رایگان نزدیک شده‌اید",
    "cart.amountLeft": "{amount} باقی مانده",
    "cart.includesDelivery": "شامل {amount} هزینه تحویل",
    "cart.checkout": "ادامه به پرداخت",
    "auth.signIn": "ورود",
    "auth.createAccount": "ایجاد حساب",
    "auth.welcomeBack": "خوش آمدید",
    "auth.join": "عضویت در {company}",
    "auth.signInTitle": "ورود به حساب شما",
    "auth.createTitle": "ایجاد حساب مشتری",
    "auth.loginDescription": "ایمیل یا شماره تلفن خود را وارد کنید.",
    "auth.registerDescription": "حساب شما با نوع مشتری عمومی ساخته می‌شود و مدیر می‌تواند آن را تغییر دهد.",
    "auth.identifier": "ایمیل یا تلفن",
    "auth.phone": "شماره تلفن",
    "auth.wait": "لطفاً منتظر بمانید...",
    "auth.hidePassword": "پنهان کردن رمز",
    "auth.showPassword": "نمایش رمز",
    "account.customerAccount": "حساب مشتری",
    "account.customerType": "نوع مشتری",
    "account.typeDescription": "این نوع، قیمت محصولات شما را تعیین می‌کند.",
    "account.phoneDescription": "برای شناسایی و پیگیری سفارش‌ها استفاده می‌شود.",
    "account.orders": "سفارش‌ها",
    "account.ordersDescription": "سفارش‌های متصل به این حساب.",
    "account.orderHistory": "تاریخچه سفارش",
    "account.orderNumbers": "شماره‌های سفارش شما",
    "account.orderNumbersDescription": "دیگر نیازی نیست شماره سفارش‌ها را دستی نگهداری کنید.",
    "account.loadingOrders": "در حال بارگذاری سفارش‌ها...",
    "account.ordersError": "سفارش‌های شما بارگذاری نشد.",
    "account.noOrders": "هنوز سفارشی ندارید",
    "account.noOrdersDescription": "سفارش بعدی شما اینجا نمایش داده می‌شود.",
    "account.startShopping": "شروع خرید",
    "checkout.title": "پرداخت",
    "checkout.secure": "پرداخت امن",
    "checkout.completeOrder": "سفارش خود را تکمیل کنید",
    "checkout.serverCheck": "پیش از ثبت سفارش، قیمت و موجودی دوباره توسط سرور بررسی می‌شود.",
    "checkout.signedIn": "واردشده به نام {name} · قیمت {type}",
    "checkout.backToCart": "بازگشت به سبد",
    "checkout.recipient": "نام گیرنده",
    "checkout.addressLine1": "سرک، ناحیه، تعمیر، شماره خانه",
    "checkout.addressLine2": "نشانی یا آپارتمان (اختیاری)",
    "checkout.city": "شهر",
    "checkout.state": "ولایت / منطقه",
    "checkout.country": "کشور",
    "checkout.postalCode": "کد پستی",
    "checkout.bankReference": "شماره مرجع انتقال بانکی",
    "checkout.completeRequired": "تمام معلومات ضروری تماس و تحویل را تکمیل کنید.",
    "checkout.enterReference": "شماره مرجع انتقال بانکی را وارد کنید.",
    "checkout.contact": "اطلاعات تماس",
    "checkout.contactDescription": "برای به‌روزرسانی سفارش و تحویل استفاده می‌شود.",
    "checkout.address": "آدرس تحویل",
    "checkout.addressDescription": "یک نسخه از این آدرس با سفارش ذخیره می‌شود.",
    "checkout.paymentMethod": "روش پرداخت",
    "checkout.paymentDescription": "پرداخت هنگام تحویل یا انتقال بانکی را انتخاب کنید.",
    "checkout.cash": "پرداخت هنگام تحویل",
    "checkout.cashDescription": "هنگام رسیدن سفارش به تحویل‌دهنده پرداخت کنید.",
    "checkout.bank": "انتقال بانکی",
    "checkout.bankDescription": "انتقال را انجام دهید و منتظر تأیید مدیر بمانید.",
    "checkout.notes": "یادداشت سفارش",
    "checkout.notesDescription": "راهنمای اختیاری برای مدیر یا تحویل‌دهنده.",
    "checkout.orderSummary": "خلاصه سفارش",
    "checkout.estimatedTotal": "مجموع تخمینی",
    "checkout.placeOrder": "ثبت سفارش",
    "checkout.creating": "در حال ثبت سفارش...",
    "orders.track": "پیگیری سفارش",
    "orders.orderNumber": "شماره سفارش",
    "orders.phoneNumber": "شماره تلفن",
    "orders.recent": "سفارش‌های اخیر در این مرورگر",
    "orders.recentDescription": "یکی را برای پیگیری سریع انتخاب کنید.",
    "orders.timeline": "خط زمانی",
    "orders.copy": "کپی",
    "orders.copied": "کپی شد",
    "orders.continueShopping": "ادامه خرید",
    "orders.trackingEyebrow": "پیگیری سفارش",
    "orders.trackingTitle": "وضعیت سفارش خود را بررسی کنید",
    "orders.trackingDescription": "شماره سفارش و همان شماره تلفنی را وارد کنید که هنگام پرداخت استفاده کردید.",
    "orders.accountTrackingDescription": "شماره سفارش خود را وارد کنید. حساب واردشده شما سفارش را به‌صورت امن تأیید می‌کند.",
    "orders.trackAction": "پیگیری",
    "orders.loading": "سفارش در حال بارگذاری است...",
    "orders.loadError": "سفارش بارگذاری نشد.",
    "orders.fulfillment": "تکمیل سفارش",
    "orders.noTimeline": "هنوز به‌روزرسانی در خط زمانی وجود ندارد.",
    "orders.received": "سفارش دریافت شد",
    "orders.thankYou": "از سفارش شما سپاسگزاریم.",
    "orders.reservedDescription": "محصولات شما رزرو شده‌اند. تیم مدیریت سفارش را بررسی و برای پردازش آماده می‌کند.",
    "orders.submittedTitle": "سفارش شما ثبت شد",
    "orders.submittedDescription": "برای پیگیری، از شماره سفارش و شماره تلفن پرداخت استفاده کنید.",
    "orders.yourNumber": "شماره سفارش شما",
    "orders.savedAccount": "این سفارش در تاریخچه سفارش‌های حساب شما ذخیره شده است.",
    "orders.savedBrowser": "این سفارش در این مرورگر ذخیره شده است. شماره را نیز کپی کنید.",
    "orders.orderStatus": "وضعیت سفارش",
    "orders.orderTotal": "مجموع سفارش",
    "orders.bankReview": "بررسی انتقال بانکی",
    "orders.cashDelivery": "پرداخت هنگام تحویل",
    "orders.trackThis": "پیگیری این سفارش",
    "orders.viewMine": "مشاهده سفارش‌های من",
    "orders.completeBank": "انتقال بانکی را تکمیل کنید",
    "orders.next": "مرحله بعد چیست؟",
    "orders.bank": "بانک",
    "orders.accountName": "نام حساب",
    "orders.accountNumber": "شماره حساب",
    "orders.bankVerify": "مدیر باید انتقال را پیش از تأیید سفارش بررسی کند.",
    "orders.stepReview": "مدیر سفارش را بررسی و موجودی را تأیید می‌کند.",
    "orders.stepProcess": "سفارش وارد مرحله پردازش و تحویل می‌شود.",
    "orders.stepCash": "هنگام تحویل سفارش به‌صورت نقدی پرداخت کنید.",
    "common.product": "محصول",
    "common.productsLower": "محصول",
    "common.itemCount": "{count} قلم",
    "category.browse": "مرور دسته‌بندی‌ها",
    "category.departments": "بخش‌های محصولات",
    "category.choose": "برای دیدن محصولات، یک دسته‌بندی را انتخاب کنید.",
    "category.featured": "دسته‌بندی ویژه",
    "category.exploreRelated": "محصولات و زیردسته‌های مرتبط را مرور کنید.",
    "category.viewAll": "مشاهده همه",
    "category.browseName": "مرور {name}",
    "category.noSubcategories": "این دسته‌بندی هنوز زیردسته ندارد؛ می‌توانید همه محصولات موجود را ببینید.",
    "category.productsInsteadOfSubcategories": "این دسته زیردسته ندارد، بنابراین محصولات موجود نمایش داده می‌شوند.",
    "category.subcategories": "زیردسته‌ها",
    "category.noProductsConfigured": "هنوز محصولی در این دسته موجود نیست.",
    "category.viewProducts": "مشاهده محصولات",
    "category.loading": "دسته‌بندی‌ها در حال بارگذاری…",
    "category.preparing": "کاتالوگ محصولات آماده می‌شود.",
    "category.empty": "دسته‌بندی موجود نیست",
    "category.emptyHelp": "پس از تنظیم دسته‌بندی‌ها در کاتالوگ، در اینجا نمایش داده می‌شوند.",
    "auth.optionalAccount": "دسترسی تأییدشده مشتری",
    "auth.heroTitle": "قیمت مناسب، سفارش‌ها و اعلان‌های شما در یک‌جا.",
    "auth.heroDescription": "با گوگل وارد شوید یا حساب مشتری بسازید و پیش از پرداخت ایمیل خود را تأیید کنید. ورود با گوگل ایمیل را خودکار تأیید می‌کند و قیمت مشتری و تاریخچه سفارش‌ها به حساب شما وصل می‌ماند.",
    "auth.pricingBenefit": "قیمت بر اساس نوع مشتری",
    "auth.pricingBenefitHelp": "ویژه، عمده، کارمند یا هر نوعی که فروشگاه تنظیم کند.",
    "auth.checkoutBenefit": "پرداخت تأییدشده توسط سرور",
    "auth.checkoutBenefitHelp": "قیمت و موجودی همیشه دوباره توسط سرور محاسبه می‌شود.",
    "auth.historyBenefit": "تاریخچه سفارش‌ها",
    "auth.historyBenefitHelp": "شماره سفارش‌های شما پس از پرداخت نیز محفوظ می‌ماند.",
    "auth.optionalHelp": "داشتن حساب اختیاری است؛ هر زمان می‌توانید این صفحه را ببندید و به‌صورت مهمان خرید کنید.",
    "auth.requestError": "درخواست حساب تکمیل نشد.",
    "auth.optionalEmail": "اختیاری، اما پیشنهاد می‌شود",
    "auth.identifierPlaceholder": "you@example.com یا 07xxxxxxxx",
    "auth.passwordPlaceholder": "حداقل ۶ نویسه",
    "auth.secureAccess": "دسترسی امن مشتری",
    "auth.backToStore": "بازگشت به خرید",
    "cart.itemsInCart": "{count} محصول در سبد شما",
    "cart.each": "برای هر واحد",
    "cart.itemTotal": "مجموع قلم",
    "cart.quantityInput": "تعداد {name}",
    "cart.increaseQuantity": "افزایش تعداد",
    "cart.decreaseQuantity": "کاهش تعداد",
    "cart.currentQuantity": "تعداد فعلی: {count}",
    "cart.quantityStep": "گام {count}",
    "cart.setQuickQuantity": "تعداد را به {count} تنظیم کنید",
    "cart.itemUnavailable": "این محصول فعلاً موجود نیست.",
    "cart.quantityUnavailable": "با موجودی فعلی فقط {count} قابل سفارش است.",
    "cart.resolveAvailability": "بررسی سبد",
    "cart.stockChanged": "موجودی یک یا چند محصول تغییر کرده است. پیش از پرداخت سبد را اصلاح کنید.",
    "cart.checkoutDisclaimer": "مالیات و جزئیات تحویل هنگام پرداخت نهایی می‌شود.",
    "catalog.searchAction": "جستجو",
    "catalog.filterProducts": "فیلتر محصولات",
    "catalog.filterDescription": "کاتالوگ را با جزئیات محصول محدود کنید.",
    "catalog.showProducts": "نمایش محصولات",
    "catalog.newest": "جدیدترین",
    "catalog.nameAsc": "نام الف–ی",
    "catalog.priceLow": "قیمت: کم به زیاد",
    "catalog.priceHigh": "قیمت: زیاد به کم",
    "catalog.activeFilters": "فیلترهای فعال:",
    "catalog.clearAll": "پاک‌کردن همه",
    "catalog.showingPage": "نمایش {count} محصول در این صفحه",
    "catalog.pageOf": "صفحه {page} از {pages}",
    "catalog.showingLoaded": "نمایش {count} از {total} محصول",
    "catalog.pinnedFirst": "{count} محصول در بالا سنجاق شده",
    "catalog.loadMore": "بارگذاری محصولات بیشتر",
    "catalog.loadingMore": "در حال بارگذاری محصولات بیشتر...",
    "catalog.allLoaded": "به پایان فهرست محصولات رسیدید.",
    "catalog.loadError": "محصولات فعلاً بارگذاری نشد. کمی بعد دوباره تلاش کنید.",
    "checkout.loadingBank": "راهنمای بانک در حال بارگذاری…",
    "checkout.bankMissing": "اطلاعات حساب بانکی تنظیم نشده است.",
    "checkout.itemsCount": "{count} قلم",
    "checkout.inventoryVerified": "سرور قیمت فعلی را بررسی و موجودی را به‌صورت اتمی رزرو می‌کند.",
    "checkout.adminApproval": "تأیید نهایی پرداخت و وضعیت تحویل توسط تیم مدیریت انجام می‌شود.",
    "account.guestTracking": "پیگیری مهمان",
    "notFound.backHome": "بازگشت به خانه",
    "common.shopNow": "اکنون خرید کنید",
    "footer.locations": "موقعیت‌های ما",
    "footer.onlineStore": "فروشگاه آنلاین مورد اعتماد شما",
    "home.categoryProducts": "محصولات محبوب",
    "home.goToSlide": "رفتن به اسلاید {number}",
    "home.nextSlide": "اسلاید بعدی",
    "home.noCategoryProducts": "محصولات به‌زودی اضافه می‌شوند",
    "home.noCategoryProductsHelp": "دسته‌بندی را باز کنید تا محصولات موجود و موارد آینده را ببینید.",
    "home.openCategory": "باز کردن {category}",
    "home.previousSlide": "اسلاید قبلی",
    "home.subcategories": "مرور زیردسته‌ها",
    "mobile.openMenu": "باز کردن منو",
    "search.allCatalog": "جستجو در تمام کاتالوگ",
    "search.categories": "دسته‌بندی‌ها",
    "search.inCategory": "جستجو در {category}",
    "search.liveResults": "جستجوی زنده",
    "search.noCategories": "دسته‌بندی مطابق پیدا نشد",
    "search.products": "محصولات",
    "search.typeMore": "برای جستجو تایپ کنید",
    "search.typeMoreHelp": "حداقل دو حرف وارد کنید تا نتایج زنده محصول و دسته‌بندی نمایش داده شود.",
    "search.viewAll": "مشاهده همه نتایج جستجو",
    "footer.contactNow": "تماس با ما",
    "footer.whatsappNow": "واتساپ",
    "header.cartEmpty": "سبد خرید خالی است",
    "header.deliverTo": "تحویل به",
    "header.savedItems": "موارد ذخیره‌شده شما",
    "header.selectLocation": "محل تحویل را انتخاب کنید",
    "header.signInRegister": "ورود / ثبت‌نام",
    "home.branchDescription": "برای تأیید نزدیک‌ترین شعبه فعال و محدوده تحویل با شرکت تماس بگیرید.",
    "home.customerSupport": "پشتیبانی مشتری",
    "home.deliveryTracking": "همراه با پیگیری سفارش",
    "home.findNearestBranch": "نزدیک‌ترین شعبه را پیدا کنید",
    "home.heroFallbackDescription": "محصولات قابل اعتماد را با قیمت روشن، موجودی زنده، پرداخت امن و تحویل مطمئن خریداری کنید.",
    "home.heroFallbackTitle": "راهی بهتر برای خرید روزانه.",
    "home.needHelpChoosing": "برای انتخاب محصول کمک نیاز دارید؟",
    "home.productCount": "{count} محصول",
    "home.storeLocations": "موقعیت‌های فروشگاه",
    "home.supportDescription": "برای موجودی محصول، سفارش یا کمک در تحویل با تیم فروشگاه تماس بگیرید.",
    "pwa.offlineDescription": 'حالت آفلاین: صفحه\u200cها و محصولاتی که قبلاً باز شده\u200cاند در دسترس می\u200cمانند. اطلاعات جدید به اتصال نیاز دارد.',
    "auth.orContinueWithPassword": "یا با رمز عبور ادامه دهید",
    "account.verifyContact": "تأیید ایمیل",
    "account.verifyDescription": "پیش از ثبت سفارش، ایمیل شما باید تأیید شود. ورود با گوگل ایمیل را به‌صورت خودکار تأیید می‌کند.",
    "account.checkoutReady": "آماده ثبت سفارش",
    "account.verificationRequired": "تأیید لازم است",
    "account.verifyEmail": "تأیید ایمیل",
    "account.verified": "تأیید شده",
    "account.verificationCode": "کد تأیید",
    "account.confirmCode": "تأیید کد",
    "account.alreadyVerified": "این ایمیل قبلاً تأیید شده است.",
    "account.codeSent": "کد تأیید به {destination} ارسال شد.",
    "account.developmentCode": "حالت توسعه: ارائه‌دهنده ارسال تنظیم نشده است. از کد {code} استفاده کنید.",
    "account.verificationComplete": "ایمیل شما موفقانه تأیید شد.",
    "account.verificationError": "تأیید انجام نشد.",
    "checkout.verifyBeforeOrder": "پیش از سفارش حساب خود را تأیید کنید",
    "checkout.verifyBeforeOrderDescription": "پیش از ثبت سفارش، ایمیل حساب مشتری را تأیید کنید. ورود با گوگل ایمیل را به‌صورت خودکار تأیید می‌کند.",
    "checkout.openVerification": "تأیید حساب",
    "common.fullName": "نام کامل",
    "common.edit": "ویرایش",
    "common.saving": "در حال ذخیره...",
    "common.saveChanges": "ذخیره تغییرات",
    "auth.forgotPassword": "رمز عبور را فراموش کرده‌اید؟",
    "auth.passwordRecovery": "بازیابی رمز عبور",
    "auth.forgotTitle": "بازنشانی رمز عبور",
    "auth.forgotDescription": "ایمیل حساب مشتری خود را وارد کنید. یک لینک امن برای بازنشانی رمز عبور برایتان ارسال می‌کنیم.",
    "auth.resetEmailPlaceholder": "you@example.com",
    "auth.resetEmailSent": "اگر حساب مشتری با این ایمیل وجود داشته باشد، لینک بازنشانی رمز عبور ارسال شده است.",
    "auth.sendResetLink": "ارسال لینک بازنشانی",
    "auth.backToSignIn": "بازگشت به ورود",
    "auth.resetLinkInvalid": "این لینک بازنشانی رمز عبور ناقص است یا دیگر اعتبار ندارد.",
    "auth.resetCompleteTitle": "رمز عبور تغییر کرد",
    "auth.resetCompleteDescription": "رمز عبور شما با موفقیت بازنشانی شد. اکنون می‌توانید با ایمیل و رمز جدید وارد شوید.",
    "auth.createNewPassword": "رمز عبور جدید بسازید",
    "auth.resetForEmail": "برای {email} یک رمز عبور جدید بسازید.",
    "auth.newPassword": "رمز عبور جدید",
    "auth.confirmPassword": "تأیید رمز عبور",
    "auth.resetPasswordAction": "بازنشانی رمز عبور",
    "account.profileInformation": "اطلاعات حساب",
    "account.profileInformationHelp": "نام و اطلاعات تماس حساب مشتری خود را به‌روز نگه دارید.",
    "account.fullNameRequired": "نام کامل الزامی است.",
    "account.profileUpdated": "اطلاعات حساب شما با موفقیت به‌روز شد.",
    "account.profileUpdateError": "اطلاعات حساب به‌روز نشد.",
    "account.contactChangeHelp": "پس از تغییر ایمیل، برای خرید دوباره باید آن را تأیید کنید. شماره تلفن فقط برای اطلاعات تماس استفاده می‌شود.",
    "account.contactStatus": "وضعیت تأیید ایمیل",
    "account.changePassword": "تغییر رمز عبور",
    "account.createPassword": "ساخت رمز عبور",
    "account.changePasswordHelp": "با وارد کردن رمز فعلی، یک رمز جدید و امن برای حساب خود تعیین کنید.",
    "account.googlePasswordHelp": "این حساب با گوگل ساخته شده و هنوز رمز عبور محلی ندارد. با ساخت رمز، می‌توانید با ایمیل نیز وارد شوید.",
    "account.currentPassword": "رمز عبور فعلی",
    "account.passwordsDoNotMatch": "رمزهای عبور یکسان نیستند.",
    "account.passwordMinimum": "حداقل ۶ کاراکتر همراه با حرف بزرگ، حرف کوچک و عدد استفاده کنید.",
    "account.passwordChanged": "رمز عبور شما با موفقیت تغییر کرد.",
    "account.passwordCreated": "رمز عبور ساخته شد. اکنون می‌توانید با گوگل یا ایمیل و رمز عبور وارد شوید.",
    "account.passwordUpdateError": "رمز عبور به‌روز نشد.",
    "account.updatePassword": "تغییر رمز عبور",
    "account.createPasswordAction": "ساخت رمز عبور",
};

...
export async function saveManualRuntimeConfig(apiUrl: string, assetUrl?: string) {
  const config = createConfig(apiUrl, assetUrl, 'manual');
  await AsyncStorage.setItem(manualConfigKey, JSON.stringify({
...
export async function testRuntimeApi(apiUrl: string) {
  ...
  try {
    ...
    if (!payload || typeof payload !== 'object') throw new Error('The server response is not valid EasyCart data.');
    return normalized;
  } catch (error) {
    if (error instanceof Error && error.name === 'AbortError') {
      throw new Error('The server did not respond within 8 seconds.');
    }
    throw error instanceof Error ? error : new Error('The server could not be reached.');
  } finally {
    clearTimeout(timer);
  }
}
...
export function normalizeApiUrl(value: string) {
  ...
  try {
    ...
    throw new Error('Enter a complete address such as https://shop.example.com/api.');
  }
  ...
  if (url.username || url.password || url.search || url.hash) {
    throw new Error('The server address cannot contain credentials, a query, or a fragment.');
  }

  const path = url.pathname.replace(/\/+$/, '');
  url.pathname = /\/api$/i.test(path) ? path : `${path}/api`.replace(/\/+/g, '/');
  return url.toString().replace(/\/+$/, '');
}

function createConfig(apiUrl: string, assetUrl: string | undefined, source: RuntimeConfigSource): RuntimeConfig {
...
async function loadRuntimeConfig() {
  ...
  if (remoteConfigUrl) {
    ...
    if (remote) {
      currentConfig = createConfig(remote.apiUrl, remote.assetUrl, 'remote');
      return currentConfig;
    }
  }
...
async function fetchRemoteConfig() {
  ...
  try {
    ...
    if (typeof payload.apiUrl !== 'string') throw new Error('Runtime configuration is missing apiUrl.');
    const validated = createConfig(payload.apiUrl, payload.assetUrl, 'remote');
    const stored = { apiUrl: validated.apiUrl, assetUrl: validated.assetUrl } satisfies StoredRuntimeConfig;
    await AsyncStorage.setItem(remoteConfigCacheKey, JSON.stringify(stored));
    return stored;
  } finally {
    clearTimeout(timer);
  }
